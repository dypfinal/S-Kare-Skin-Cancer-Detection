...
def stem_tfidf(doc, query):
    ...
    tf = TfidfVectorizer(
        use_idf=True, sublinear_tf=True, stop_words=stopwords.words("english")
    )
...
while stop == True:
    ...
    while stop1 == True:
        query = input(
            "\nHi "
            + (newname if len(newname) != 0 else name)
            + ", I am Nate. How can I help you? If you want to exit, type Bye. :"
        )
        query = query.lower()
        query = query.strip("!@#$%^&*()<>,;?")
        if query == "bye":
            stop1 = False
            print(
                "\nNate: This is Nate signing off. Bye, take care"
                + (newname if len(newname) != 0 else name)
            )
            ...
            or query == "what is my name"
            or query == "whats my name?"
            or query == "whats my name"
        ):
            if len(newname) != 0:
                print("\nNate: Your name is " + newname)
